		...
		<table class="table">
			...
			<tbody>
				...
				<tr>
					<td>${status.count }</td>
					<td>
						<a href="${path1 }/ProductDetail.do?gcode=${sale.gcode }" title="${sale.gcode }">${sale.gname }</a>
					</td>
					...
					<td>
						구매금액 : <span><fmt:formatNumber value="${sale.price }" pattern="#,##0 원" /></span><br>
						판매금액 : <span><fmt:formatNumber value="${sale.price }" pattern="#,##0 원" /></span>
						<c:set var="total1" value="${total1 + (sale.price)}"/>
